...
`config/user-tool-paths.json` is the one supported way to point a launcher at a binary outside the bundled tree. It is per-machine, untracked and unsigned, so it is treated as untrusted input at launch time rather than only when it is written. A launcher backed by a user-declared path outside the tools tree requires an explicit acknowledgement before it starts, even when its catalog risk tier is read-only: `-AcknowledgeRisk` on the command line, or a confirmation dialog in the console. The prompt states the resolved path, whether it sits inside the tools tree, and its Authenticode status.

Containment in the tools tree is what decides whether the extra prompt appears. A valid signature is reported but deliberately not required, because many of the utilities this console exists to run ship unsigned (Prime95, y-cruncher, TestMem5, HCI MemTest, H2testw, several NirSoft tools); requiring one would prompt on nearly every legitimate override and train the operator to click through the warning. Containment is also the property `scripts/Test-ProgramConnections.ps1` already enforces when it validates a generated plan. This does not make an untrusted binary safe; it removes the silent case where a repointed read-only tool would launch with no prompt at all.
